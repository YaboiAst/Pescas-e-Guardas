...
public class BuoyantObject : MonoBehaviour
{
    ...
    private void FixedUpdate()
    {
        ...
        for (var i = 0; i < effectorAmount; i++)
        {
            ...
            if (!(effectorHeight < waveHeight)) continue; 
    ...
    private void OnDrawGizmos()
    {
        ...
        {
            if (!Application.isPlaying && effectors[i])
            {
                Gizmos.color = green;
                Gizmos.DrawSphere(effectors[i].position, 0.5f);
            }
            ...
            else
            {
                ...
                Gizmos.color = orange;
                Gizmos.DrawSphere(effectorProjections[i], 0.5f);
